...
export interface PacedPlan {
  /** Total duration to run the paced calls. */
  duration_s: number;
  /** Seconds between each call. */
  tick_s: number;
  /** Endpoint sequence (round-robin) to call on each tick. */
  sequence: Endpoint[];
}
...
export interface Scenario {
  ...
  id: string;
  /** Optional suite label for workflow generation. */
  suite?: "self-test" | "realistic";
  ...
  /** Optional paced call plan for realistic, steady traffic. */
  paced_plan?: PacedPlan;
 ...
 *
 * Scenarios are executed serially (matrix with max-parallel: 1 in the
 * generated workflow, with per-scenario run toggles). They range from
 * a zero-call baseline through
...
export const SCENARIOS: Scenario[] = [
  ...
  // 13 — Realistic steady traffic mix (5 minutes)
  {
    id: "realistic-5m",
    suite: "realistic",
